...
6. **옛 WAL 정리**: 이제 REDO 위치 이전의 WAL은 장애 복구에 필요 없으므로, 그 세그먼트를 지우거나 이름을 바꿔 재활용합니다. 단 `wal_keep_size`, replication slot([9편](/posts/postgresql/09-streaming-replication/))이 요구하는 몫과 아직 보관(archive)되지 않은 세그먼트는 남깁니다([`RemoveOldXlogFiles()`](https://github.com/postgres/postgres/blob/39a0db101105eab3f4044d11c609c58b9459ea16/src/backend/access/transam/xlog.c#L3857), [7편](/posts/postgresql/07-wal/#wal-세그먼트는-재활용된다)).
...
#### pg_control: 서버가 기억하는 마지막 체크포인트

서버 로그에 프로세스 종류가 나오도록 `log_line_prefix`에 `%b`를 넣고 서버를 띄운 뒤, 20만 행짜리 테이블 `acct`를 만듭니다. `log_checkpoints`는 PG15부터 기본으로 켜져 있어 체크포인트마다 로그가 남습니다.
...
postgres=# CREATE TABLE acct (id int PRIMARY KEY, balance int, pad text);
postgres=# INSERT INTO acct SELECT g, 100, repeat('p', 200) FROM generate_series(1, 200000) g;
```
...
5만 행을 바꾼 뒤 `CHECKPOINT`를 실행합니다. [앞에서](#pg_control-서버가-기억하는-마지막-체크포인트) 20만 행을 넣은 뒤로 체크포인트가 한 번도 없었으므로, 그 INSERT로 dirty가 된 페이지도 아직 메모리에만 있습니다.

```psql
postgres=# UPDATE acct SET balance = balance + 1 WHERE id <= 50000;
postgres=# SELECT num_timed, num_requested, num_done, buffers_written FROM pg_stat_checkpointer;
...
postgres=# UPDATE acct SET balance = balance + 1;
postgres=# UPDATE acct SET balance = balance + 1;
```
...
```psql
postgres=# SELECT num_timed, num_requested, num_done FROM pg_stat_checkpointer;
 num_timed | num_requested | num_done 
-----------+---------------+----------
         0 |             9 |        8
(1 row)

postgres=# ALTER SYSTEM RESET max_wal_size;
postgres=# SELECT pg_reload_conf();
```
...
`num_requested` 9는 [앞에서](#수동-checkpoint) 실행한 `CHECKPOINT` 명령 1번과 이번 `wal` 8번입니다. `num_requested`는 체크포인트를 시작할 때, `num_done`은 끝날 때 늘어나는데, 마지막 하나는 UPDATE가 끝나 WAL이 더 늘지 않자 시간 기준 일정에 맞춰 천천히 쓰는 중이라 `num_done`이 8입니다.

#### 운영에서는: checkpoints are occurring too frequently

이 경고는 `wal` 체크포인트가 `checkpoint_warning`(기본 30초)보다 짧은 간격으로 일어날 때 나옵니다([`checkpointer.c`](https://github.com/postgres/postgres/blob/39a0db101105eab3f4044d11c609c58b9459ea16/src/backend/postmaster/checkpointer.c#L454-L462)). 대량 적재나 배치 UPDATE 때 자주 보입니다. 체크포인트가 잦으면 체크포인트 직후마다 페이지 이미지가 쏟아지고([7편](/posts/postgresql/07-wal/)), 쓰기를 나눌 시간도 없어 디스크 I/O가 튑니다. 보통은 로그의 힌트대로 `max_wal_size`를 늘려 해결합니다. `pg_stat_checkpointer`에서 `num_requested`가 `num_timed`보다 훨씬 많다면 같은 신호입니다.
...
```psql
postgres=# ALTER SYSTEM RESET checkpoint_timeout;
postgres=# SELECT pg_reload_conf();
```
...
#### 장애 복구: 마지막 체크포인트부터 WAL을 다시 적용한다
...
```psql
postgres=# SELECT sum(balance) AS total, count(*) FROM acct;
  total   | count  
----------+--------
 20650000 | 200000
(1 row)
...
postgres=# CHECKPOINT;
postgres=# UPDATE acct SET balance = balance + 1 WHERE id <= 100000;
postgres=# INSERT INTO acct VALUES (300001, 777, 'after checkpoint');
...
postgres=# SELECT * FROM acct WHERE id = 300001;
   id   | balance |       pad        
--------+---------+------------------
...
### 같은 변경을 두 번 적용하지 않는 장치: 페이지 LSN

복구 4단계에서 같은 변경을 두 번 적용하지 않는 장치가 **페이지 LSN**입니다. 레코드가 건드리는 페이지를 읽었을 때,
...
#### 복구 시간은 마지막 체크포인트 이후 WAL 양에 비례한다
...
postgres=# ALTER SYSTEM SET checkpoint_timeout = '1h';
postgres=# SELECT pg_reload_conf();
postgres=# CHECKPOINT;
postgres=# UPDATE acct SET balance = balance + 1;
postgres=# UPDATE acct SET balance = balance + 1;
...
postgres=# UPDATE acct SET balance = balance + 1;
postgres=# UPDATE acct SET balance = balance + 1;
postgres=# SELECT pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_insert_lsn(), (SELECT redo_lsn FROM pg_control_checkpoint()))) AS wal_since_redo;
 wal_since_redo 
----------------
 453 MB
(1 row)
...
```psql
postgres=# INSERT INTO acct VALUES (400001, 1, 'before mistake');
postgres=# SELECT pg_create_restore_point('before_drop');
 pg_create_restore_point 
-------------------------
 0/52001CD0
(1 row)

postgres=# DROP TABLE acct;
...
```psql
postgres=# SELECT archived_count, last_archived_wal, failed_count FROM pg_stat_archiver;
...
백업 디렉터리를 복원용 서버(포트 5433)로 켜서, 복원 지점까지만 복구합니다. 프롬프트 `restored=#`는 복원한 서버(포트 5433), `original=#`는 원래 서버(포트 5432)입니다.
...
```psql
restored=# SELECT count(*), max(id) FROM acct;
 count  |  max   
--------+--------
 200002 | 400001
(1 row)

original=# SELECT count(*) FROM acct;
...
#### 운영에서는: PITR은 연습해 둬야 한다

PITR에는 베이스 백업과, 그 백업 이후 **끊김 없는** WAL 보관본이 모두 필요합니다. `pg_stat_archiver`의 `failed_count`를 모니터링하고 복원 절차는 실제로 한 번 해 봐야 합니다. 위 실습처럼 위험한 작업 전에 `pg_create_restore_point()`로 이름을 붙여 두면 멈출 지점을 찾기 쉽습니다.
