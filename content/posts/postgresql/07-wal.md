...
서버를 띄우고, 이 글 내내 쓸 `acct` 테이블을 만든 뒤 `CHECKPOINT`를 해 둡니다. WAL 레코드를 보는 데는 contrib 확장 [pg_walinspect](https://www.postgresql.org/docs/18/pgwalinspect.html)와 명령줄 도구 [pg_waldump](https://www.postgresql.org/docs/18/pgwaldump.html)를 씁니다.
...
postgres=# CREATE TABLE acct (id int PRIMARY KEY, balance int, memo text);
postgres=# INSERT INTO acct SELECT g, 100, 'init' FROM generate_series(1, 1000) g;
postgres=# CHECKPOINT;
...
postgres=# SELECT count(*) AS segments, pg_size_pretty(sum(size)) AS total FROM pg_ls_waldir();
 segments | total 
----------+-------
        4 | 64 MB
(1 row)

postgres=# CREATE TABLE bulk AS SELECT g AS id, repeat('w', 200) AS pad FROM generate_series(1, 400000) g;
postgres=# SELECT count(*) AS segments, pg_size_pretty(sum(size)) AS total FROM pg_ls_waldir();
 segments | total  
----------+--------
       10 | 160 MB
...
postgres=# SELECT pg_walfile_name(pg_current_wal_insert_lsn()) AS current_segment;
...
{{< diagram src="/diagrams/pg-wal-record.html" title="WAL 레코드 하나의 구조" height="560" caption="아래 full page write 실습에서 볼 UPDATE 레코드. 고정 헤더 뒤에 건드린 블록마다 블록 헤더가 오고, 헤더가 모두 끝난 뒤 블록 데이터와 main data가 이어집니다." >}}
...
#### INSERT 하나가 남기는 WAL 레코드

행 하나를 두 번 넣고, 각각 WAL을 얼마나 남겼는지 봅니다.

```psql
postgres=# SELECT pg_current_wal_insert_lsn() AS s1 \gset
postgres=# INSERT INTO acct VALUES (1001, 100, 'hello');
INSERT 0 1
postgres=# SELECT pg_current_wal_insert_lsn() AS e1 \gset
postgres=# INSERT INTO acct VALUES (1002, 100, 'world');
INSERT 0 1
postgres=# SELECT pg_current_wal_insert_lsn() AS e2 \gset
postgres=# SELECT 'first' AS insert, pg_wal_lsn_diff(:'e1', :'s1') AS wal_bytes
postgres-# UNION ALL SELECT 'second', pg_wal_lsn_diff(:'e2', :'e1');
 insert | wal_bytes 
--------+-----------
 first  |      8968
 second |       176
...
postgres=# SELECT start_lsn, xid, resource_manager AS rmgr, record_type, record_length AS len, fpi_length AS fpi, description
...
- 첫 번째 INSERT는 [앞에서](#lsn과-wal-세그먼트-파일) 테이블을 만들고 한 `CHECKPOINT` 뒤로 이 테이블 페이지와 인덱스 페이지를 처음 고친 것이라, 두 레코드 모두 페이지 이미지를 달고 있습니다(`fpi` 3368, 5420). 레코드 세 개는 힙에 튜플 넣기(`Heap INSERT`), 인덱스에 항목 넣기(`Btree INSERT_LEAF`), 커밋(`Transaction COMMIT`)입니다.
...
initdb 이후 쌓인 WAL 전체를 resource manager별로 집계합니다. 뒤의 [pgbench 실습](#synchronous_commit에-따른-처리량-차이)까지 마친 뒤의 집계입니다.

```psql
postgres=# SELECT "resource_manager/record_type" AS rmgr, count, round(count_percentage::numeric, 1) AS count_pct,
postgres-#        pg_size_pretty(combined_size) AS bytes, round(combined_size_percentage::numeric, 1) AS bytes_pct,
postgres-#        round(fpi_size_percentage::numeric, 1) AS fpi_pct
postgres-# FROM pg_get_wal_stats('0/1000000', pg_current_wal_lsn())
...
CHECKPOINT
postgres=# SELECT pg_current_wal_insert_lsn() AS s1 \gset
postgres=# UPDATE acct SET balance = balance + 1 WHERE id = 1;
UPDATE 1
postgres=# SELECT pg_current_wal_insert_lsn() AS e1 \gset
postgres=# UPDATE acct SET balance = balance + 1 WHERE id = 2;
UPDATE 1
postgres=# SELECT pg_current_wal_insert_lsn() AS e2 \gset
postgres=# SELECT 'first update after checkpoint' AS which, resource_manager AS rmgr, record_type, record_length AS len, fpi_length AS fpi, block_ref
postgres-# FROM pg_get_wal_records_info(:'s1', :'e1')
postgres-# UNION ALL
postgres-# SELECT 'second update (same page)', resource_manager, record_type, record_length, fpi_length, block_ref
postgres-# FROM pg_get_wal_records_info(:'e1', :'e2');
...
#### full_page_writes와 wal_compression이 WAL 양에 주는 영향
...
postgres=# ALTER SYSTEM SET wal_compression = 'pglz';
...
(1 row)
postgres=# ALTER SYSTEM RESET full_page_writes;
postgres=# ALTER SYSTEM RESET wal_compression;
postgres=# SELECT pg_reload_conf();
```
...
#### 운영에서는: 체크포인트 직후 WAL이 치솟는다

[체크포인트 뒤 첫 수정](#full-page-write-체크포인트-뒤-첫-수정)과 위 실습에서 본 것처럼, 체크포인트 직후에는 모든 페이지가 "처음 고치는 페이지"라 페이지 이미지가 쏟아집니다. 쓰기가 많은 시스템에서 체크포인트마다 WAL 생성량이 톱니 모양으로 치솟는 것이 이 때문입니다. 체크포인트 간격(`checkpoint_timeout`, `max_wal_size`)을 늘리면 이미지가 덜 생기고, `wal_compression`을 켜면 크기가 줄어듭니다. 대신 체크포인트 간격이 길수록 장애 복구 시간은 길어집니다([8편](/posts/postgresql/08-checkpoint-and-recovery/)).

### hint bit만 바꿔도 이미지가 남는다
...
postgres=# INSERT INTO acct SELECT g, 100, 'new' FROM generate_series(2001, 2200) g;
INSERT 0 200
postgres=# CHECKPOINT;
CHECKPOINT
postgres=# SELECT pg_current_wal_insert_lsn() AS s \gset
postgres=# SELECT count(*) FROM acct WHERE id > 2000;
 count 
-------
   200
(1 row)

postgres=# SELECT pg_current_wal_insert_lsn() AS e \gset
postgres=# SELECT pg_wal_lsn_diff(:'e', :'s') AS wal_bytes_by_select,
postgres-#        pg_wal_lsn_diff(:'e', pg_current_wal_lsn()) AS not_yet_written;
...
postgres=# CHECKPOINT;
CHECKPOINT
postgres=# SELECT resource_manager AS rmgr, record_type, count(*), sum(fpi_length) AS fpi_bytes
postgres-# FROM pg_get_wal_records_info(:'s', :'e') GROUP BY 1, 2;
...
postgres=# SELECT record_type, fpi_length AS fpi, block_ref FROM pg_get_wal_records_info(:'s', :'e') WHERE fpi_length > 0;
...
postgres=# SELECT (ctid::text::point)[0]::int AS blk, count(*) AS new_rows FROM acct WHERE id > 2000 GROUP BY 1 ORDER BY 1;
 blk | new_rows 
-----+----------
   5 |      106
   6 |       94
...
postgres=# EXPLAIN (COSTS OFF) SELECT count(*) FROM acct WHERE id > 2000;
         QUERY PLAN          
-----------------------------
 Aggregate
   ->  Seq Scan on acct
...
#### 운영에서는: 읽기만 하는데 WAL이 생긴다

대량 적재 직후나 복구 직후에 SELECT만 도는데 WAL이 계속 생긴다면, 위 실습에서 본 hint bit 이미지(`FPI_FOR_HINT`)일 가능성이 큽니다. PG18에서 새로 만든 클러스터는 체크섬이 기본으로 켜져 있으므로 이 현상이 전보다 흔할 수 있습니다. 대량 적재 뒤 `VACUUM`을 한 번 돌려 두면 hint bit와 페이지 정리를 한꺼번에 끝낼 수 있습니다. standby가 있다면 이 WAL이 그대로 복제 트래픽이 된다는 점도 기억해 둘 만합니다.
