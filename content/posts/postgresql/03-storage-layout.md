...
아래 그림은 실습에서 만든 `fruit` 테이블의 첫 페이지를 그대로 그린 것입니다.
...
압축 방식은 `default_toast_compression`으로 정하고 기본값은 pglz입니다([`TOAST_PGLZ_COMPRESSION`](https://github.com/postgres/postgres/blob/39a0db101105eab3f4044d11c609c58b9459ea16/src/backend/access/common/toast_compression.c#L26)). `lz4`도 쓸 수 있지만 빌드할 때 `--with-lz4`가 필요합니다. 이 글의 실습 이미지는 lz4 없이 빌드해서 pglz만 씁니다.

## 직접 확인해 보기

### 실습 환경

[2편과 같은 실습 이미지](/labs/pg-lab-image/Dockerfile)를 씁니다. 전체 실습은 [lab.sh](/labs/pg-03-storage/lab.sh)가 새 컨테이너에서 처음부터 끝까지 실행했고(공용 함수는 [labkit.sh](/labs/common/labkit.sh)), 원본 출력은 [final-run.log](/labs/pg-03-storage/final-run.log)에 있습니다. 페이지 내용을 보는 데는 contrib 확장인 [pageinspect](https://www.postgresql.org/docs/18/pageinspect.html)를 씁니다.
...
```text
1b13b5cd6424dd13a3b45ed580132f054e038746dcefb63050f615baafe0b06a
[exit=0]
```
...
initdb에 체크섬 옵션을 따로 주지 않았습니다. 그래도 체크섬이 켜진다는 것을 실습 1에서 확인합니다.
...
| `flags = 4` | [`PD_ALL_VISIBLE`](https://github.com/postgres/postgres/blob/39a0db101105eab3f4044d11c609c58b9459ea16/src/include/storage/bufpage.h#L188-L190). 방금 VACUUM이 "이 페이지의 모든 행은 모두에게 보인다"고 표시함 |
...
### 실습 7. 한 페이지에 들어가는 행 수와 fillfactor

`int` 열 하나짜리 테이블에 1만 행을 넣고, fillfactor 50으로 만든 테이블과 비교합니다.
...
실습 5처럼 고정 길이 타입을 큰 것부터(`bigint`, `timestamp` → `int` → `smallint` → `bool`) 앞에 두고 가변 길이 타입(`text` 등)을 뒤에 두면 패딩이 줄어듭니다. 이미 만든 테이블의 열 순서는 바꿀 수 없으므로, 수억 행이 쌓일 테이블이라면 설계할 때 한 번 따져 볼 만합니다. 반대로 행이 적은 테이블이라면 가독성을 위해 논리적인 순서를 두는 편이 낫습니다.

### 행 하나에도 23바이트 이상의 비용이 든다

PostgreSQL의 행은 데이터 말고도 헤더 24바이트(정렬 포함)와 line pointer 4바이트를 씁니다. `int` 하나만 있는 행이 36바이트를 차지한 실습 7이 그 예입니다. 작은 행이 아주 많은 테이블(로그, 이벤트 등)은 데이터 크기로 짐작한 것보다 디스크를 훨씬 많이 씁니다. 용량을 추정할 때는 이 고정 비용을 함께 계산해야 합니다.
...
- [실습 이미지 Dockerfile](/labs/pg-lab-image/Dockerfile), [labkit.sh](/labs/common/labkit.sh), [lab.sh](/labs/pg-03-storage/lab.sh), [final-run.log](/labs/pg-03-storage/final-run.log)
