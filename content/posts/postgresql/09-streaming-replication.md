...
standby는 `standby.signal` 파일이 있는 데이터 디렉터리로 시작하며, `primary_conninfo`에 적힌 primary로 접속합니다. `pg_basebackup -R`이 이 두 가지를 만들어 줍니다. `hot_standby`(기본 on)이면 재생하는 동안에도 읽기 전용 쿼리를 받습니다.

#### pg_basebackup -R로 standby 만들기

primary(포트 5432)와 standby(포트 5433) 두 클러스터를 한 서버에 띄웁니다. 프로세스 이름에 어느 쪽인지 나오도록 `cluster_name`을 붙였고, psql 프롬프트의 `primary=#`와 `standby=#`는 어느 서버에 접속해 실행했는지를 나타냅니다. 먼저 primary를 띄우고 테이블을 만듭니다.
...
```psql
primary=# CREATE TABLE acct (id int PRIMARY KEY, balance int, pad text);
primary=# INSERT INTO acct SELECT g, 100, repeat('p', 200) FROM generate_series(1, 200000) g;
```

primary의 베이스 백업으로 standby를 만들고, 포트와 `cluster_name`을 바꿔 띄웁니다.
...
#### pg_stat_replication과 pg_stat_wal_receiver
...
primary=# SELECT 'primary: in_recovery=' || pg_is_in_recovery() \g (format=unaligned tuples_only=on)
primary: in_recovery=false
standby=# SELECT 'standby: in_recovery=' || pg_is_in_recovery() \g (format=unaligned tuples_only=on)
...
#### primary의 변경이 standby에 보인다

```psql
primary=# INSERT INTO acct VALUES (200001, 1, 'from primary');
primary=# SELECT pg_current_wal_lsn() AS primary_lsn;
 primary_lsn 
-------------
 0/7002148
(1 row)
...
standby=# SELECT * FROM acct WHERE id = 200001;
   id   | balance |     pad      
--------+---------+--------------
 200001 |       1 | from primary
(1 row)

standby=# INSERT INTO acct VALUES (200002, 1, 'from standby');
...
```psql
standby=# ALTER SYSTEM SET recovery_min_apply_delay = '5s';
standby=# SELECT pg_reload_conf();
primary=# INSERT INTO acct VALUES (200003, 1, 'delayed');
primary=# SELECT pg_current_wal_lsn() AS primary_lsn, sent_lsn, write_lsn, flush_lsn, replay_lsn, write_lag, flush_lag, replay_lag FROM pg_stat_replication \gx
-[ RECORD 1 ]----------------
...
`pg_stat_replication`의 네 위치로 지연이 어디서 생기는지 먼저 나눕니다. `replay_lsn`만 뒤처진다면 standby의 재생이 느린 것이고, 원인은 standby의 I/O 부족이나 [쿼리 충돌](#hot-standby-충돌) 대기인 경우가 많습니다. 재생은 startup 프로세스 하나가 순서대로 하므로([8편](/posts/postgresql/08-checkpoint-and-recovery/)), primary에서 여러 세션이 병렬로 만든 WAL을 standby가 따라잡지 못할 수도 있습니다.
...
```psql
primary=# ALTER SYSTEM SET synchronous_standby_names = 'standby1';
primary=# SELECT pg_reload_conf();
primary=# SELECT application_name, sync_state FROM pg_stat_replication;
...
primary와, 다시 켠 standby에서 그 행을 찾아봅니다.

```psql
primary=# SELECT * FROM acct WHERE id = 200005;
   id   | balance |    pad    
--------+---------+-----------
 200005 |       1 | sync wait
(1 row)
...
```psql
standby=# SELECT * FROM acct WHERE id = 200005;
   id   | balance |    pad    
--------+---------+-----------
 200005 |       1 | sync wait
(1 row)

primary=# ALTER SYSTEM RESET synchronous_standby_names;
primary=# SELECT pg_reload_conf();
```
...
#### standby 쿼리와 WAL 재생의 충돌

standby의 `max_standby_streaming_delay`를 3초로 줄입니다.
...
standby=# ALTER SYSTEM SET max_standby_streaming_delay = '3s';
standby=# SELECT pg_reload_conf();
```

standby에 접속한 세션 B에서 20초 걸리는 쿼리를 돌리는 동안, primary에서 행을 지우고 VACUUM합니다.

```psql
B=# SELECT count(*), pg_sleep(20) FROM acct;
primary=# DELETE FROM acct WHERE id > 200000;
primary=# VACUUM acct;
```

세션 B에는 다음 오류가 돌아왔습니다.
...
```psql
standby=# SELECT datname, confl_snapshot FROM pg_stat_database_conflicts WHERE datname = 'postgres';
 datname  | confl_snapshot 
----------+----------------
 postgres |              1
(1 row)

standby=# ALTER SYSTEM RESET max_standby_streaming_delay;
standby=# SELECT pg_reload_conf();
```

standby의 쿼리가 `canceling statement due to conflict with recovery`로 취소되었습니다. DETAIL은 "쿼리가 봐야 할 수도 있는 행 버전을 지워야 한다"는 뜻입니다. primary의 VACUUM이 지운 행 버전(앞에서 넣었다가 DELETE한 행)의 정리 기록을 재생하려다, 그 버전을 볼 수도 있는 쿼리의 스냅샷과 부딪혔습니다. startup은 `max_standby_streaming_delay`(3초)까지 기다린 뒤 쿼리를 취소하고 재생을 계속했고, standby의 `pg_stat_database_conflicts.confl_snapshot`이 1이 되었습니다. 기본값(30초)이었다면 이 20초짜리 쿼리는 취소되지 않았을 것입니다. 대신 쿼리가 끝날 때까지 재생이 멈춰 standby가 그만큼 뒤처졌을 것입니다.

#### 운영에서는: standby 쿼리가 자주 취소된다
...
```psql
primary=# SELECT slot_name, active, restart_lsn, wal_status, pg_size_pretty(safe_wal_size) AS safe_wal_size FROM pg_replication_slots;
 slot_name | active | restart_lsn | wal_status | safe_wal_size 
-----------+--------+-------------+------------+---------------
 standby1  | f      | 0/A0543F0   | reserved   | 
(1 row)

primary=# UPDATE acct SET balance = balance + 1;
primary=# UPDATE acct SET balance = balance + 1;
primary=# UPDATE acct SET balance = balance + 1;
primary=# CHECKPOINT;
...
#### max_slot_wal_keep_size를 넘으면 slot을 포기한다

```psql
primary=# ALTER SYSTEM SET max_slot_wal_keep_size = '128MB';
primary=# SELECT pg_reload_conf();
primary=# UPDATE acct SET balance = balance + 1;
primary=# CHECKPOINT;
primary=# SELECT slot_name, active, restart_lsn, wal_status, invalidation_reason FROM pg_replication_slots;
...
#### PG18의 idle_replication_slot_timeout

아무도 쓰지 않는 slot `forgotten`을 만들고, 1초 이상 쉰 slot을 무효화하도록 설정한 뒤 2초 뒤에 체크포인트합니다.
...
primary=# SELECT pg_reload_conf();
primary=# CHECKPOINT;
primary=# SELECT slot_name, active, inactive_since IS NOT NULL AS has_inactive_since, wal_status, invalidation_reason FROM pg_replication_slots ORDER BY slot_name;
...
#### 운영에서는: 방치된 slot이 디스크를 가득 채운다

가장 흔한 복제 사고입니다. standby를 없애거나 논리 복제 구독을 지우면서 slot을 지우지 않으면, [앞에서 본 것처럼](#slot은-standby가-멈춰도-wal을-붙잡는다) `pg_wal`이 끝없이 커집니다. `pg_replication_slots`에서 `active = false`이면서 `restart_lsn`이 오래된 slot을 주기적으로 확인하고, 쓰지 않는 slot은 `pg_drop_replication_slot()`으로 지워야 합니다. 안전장치로 `max_slot_wal_keep_size`를 디스크 여유에 맞게 정해 두면, 최악의 경우 standby 하나를 다시 만드는 것으로 끝나고 primary는 멈추지 않습니다. PG18에서는 `idle_replication_slot_timeout`도 쓸 수 있습니다.
