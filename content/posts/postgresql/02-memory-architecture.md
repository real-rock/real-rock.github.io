...
postgres-# WHERE name IN ('shared_buffers', 'wal_buffers', 'work_mem', 'hash_mem_multiplier',
postgres-#                'maintenance_work_mem', 'temp_buffers', 'block_size', 'shared_memory_size')
...
```psql
postgres=# SELECT name, pg_size_pretty(allocated_size) AS size
postgres-# FROM pg_shmem_allocations
postgres-# ORDER BY allocated_size DESC
postgres-# LIMIT 10;
        name        |  size   
--------------------+---------
...
postgres=# SELECT count(*) AS entries, pg_size_pretty(sum(allocated_size)) AS total
postgres-# FROM pg_shmem_allocations;
 entries | total  
---------+--------
      73 | 150 MB
...
#### 테이블을 읽으면 shared buffers에 페이지가 올라온다

`pg_buffercache` 확장을 설치하고, 14MB(1728페이지) 테이블을 만들어 이 테이블의 페이지를 캐시에서 내보낸 뒤 한 번 읽어 봅니다. 캐시를 비우는 데는 PG18의 `pg_buffercache_evict_relation()`을 씁니다. 서버를 재시작하지 않고 특정 테이블의 페이지만 shared buffers에서 내보냅니다.

```psql
postgres=# CREATE EXTENSION pg_buffercache;
postgres=# CREATE TABLE small AS SELECT g AS id, repeat('x', 100) AS pad FROM generate_series(1, 100000) g;
SELECT 100000
postgres=# SELECT pg_size_pretty(pg_relation_size('small')) AS size, pg_relation_size('small') / 8192 AS pages;
 size  | pages 
-------+-------
 14 MB |  1728
(1 row)

postgres=# SELECT pg_buffercache_evict_relation('small');
...
postgres=# SELECT count(*) FROM small;
 count  
--------
 100000
(1 row)
...
postgres=# SELECT count(*) AS buffers_of_small
postgres-# FROM pg_buffercache
postgres-# WHERE relfilenode = pg_relation_filenode('small');
 buffers_of_small 
------------------
             1728
(1 row)
...
postgres=# EXPLAIN (ANALYZE, COSTS OFF, TIMING OFF, SUMMARY OFF) SELECT count(*) FROM small;
                                 QUERY PLAN                                  
-----------------------------------------------------------------------------
...
#### usage count: 자주 읽는 페이지일수록 오래 살아남는다
...
postgres=# SELECT usagecount, count(*) FROM pg_buffercache
postgres-# WHERE relfilenode = pg_relation_filenode('small') GROUP BY 1 ORDER BY 1;
 usagecount | count 
------------+-------
          1 |  1728
(1 row)
...
#### read는 디스크에서 읽었을까, 운영체제 캐시에서 읽었을까
...
#### 수정된 페이지(dirty)는 체크포인트가 디스크로 내보낸다

```psql
postgres=# CHECKPOINT;
CHECKPOINT
postgres=# SELECT buffers_dirty FROM pg_buffercache_summary();
 buffers_dirty 
---------------
             0
(1 row)

postgres=# UPDATE small SET pad = repeat('y', 100) WHERE id % 10 = 0;
UPDATE 10000
postgres=# SELECT buffers_dirty FROM pg_buffercache_summary();
 buffers_dirty 
---------------
...
postgres=# SELECT count(*) FILTER (WHERE isdirty) AS dirty_of_small
postgres-# FROM pg_buffercache WHERE relfilenode = pg_relation_filenode('small');
 dirty_of_small 
----------------
           1899
(1 row)

postgres=# SELECT buffers_written FROM pg_stat_checkpointer;
 buffers_written 
-----------------
             328
(1 row)
...
#### 운영에서는: shared_buffers를 무작정 키우면 안 되는 이유

shared buffers가 클수록 hit가 늘어나는 것은 맞습니다. 하지만 [앞에서 본 것처럼](#read는-디스크에서-읽었을까-운영체제-캐시에서-읽었을까) PostgreSQL은 운영체제 페이지 캐시 위에서 동작합니다. shared buffers를 너무 크게 잡으면 운영체제가 쓸 캐시가 줄어들고, 같은 페이지를 두 곳에 들고 있는 낭비도 커집니다. 또 shared buffers는 기동할 때 한 번에 잡는 고정 영역이라, 크게 잡을수록 기동할 때 확보할 메모리와 체크포인트 때 내보낼 dirty 페이지 양이 늘어납니다. PostgreSQL 문서는 전용 서버에서 **메모리의 25% 정도에서 시작**하고, 40%를 넘기면 이득이 적다고 안내합니다([shared_buffers](https://www.postgresql.org/docs/18/runtime-config-resource.html#GUC-SHARED-BUFFERS)). 실제 값은 `pg_buffercache`와 hit 비율을 보면서 조정합니다.
...
PG18에서 순차 스캔의 ring 계산이 바뀌었습니다. 비동기 I/O로 여러 블록을 미리 읽어 두려면 칸이 더 필요하기 때문에, 256kB에 `io_combine_limit × effective_io_concurrency`만큼을 더합니다. 대신 backend 하나가 pin할 수 있는 칸 수([`GetPinLimit()`](https://github.com/postgres/postgres/blob/39a0db101105eab3f4044d11c609c58b9459ea16/src/backend/storage/buffer/bufmgr.c#L2514-L2517), [`NBuffers / (MaxBackends + NUM_AUXILIARY_PROCS)`](https://github.com/postgres/postgres/blob/39a0db101105eab3f4044d11c609c58b9459ea16/src/backend/storage/buffer/bufmgr.c#L4042))를 넘지 못합니다. [아래 실습](#큰-테이블-순차-스캔은-ring-buffer만-쓴다)에서 이 값을 직접 계산해 맞춰 봅니다.
...
#### 큰 테이블 순차 스캔은 ring buffer만 쓴다

81MB(10368페이지)짜리 테이블을 만듭니다. shared buffers의 1/4인 4096페이지보다 크므로 ring buffer 대상입니다.

```psql
postgres=# CREATE TABLE big AS SELECT g AS id, repeat('x', 100) AS pad FROM generate_series(1, 600000) g;
SELECT 600000
postgres=# SELECT pg_size_pretty(pg_relation_size('big')) AS size,
postgres-#        pg_relation_size('big') / 8192 AS pages,
postgres-#        current_setting('shared_buffers') AS shared_buffers,
postgres-#        (SELECT setting::int FROM pg_settings WHERE name = 'shared_buffers') / 4 AS bulkread_threshold_pages;
 size  | pages | shared_buffers | bulkread_threshold_pages 
-------+-------+----------------+--------------------------
 81 MB | 10368 | 128MB          |                     4096
(1 row)

postgres=# SELECT pg_buffercache_evict_relation('big');
 pg_buffercache_evict_relation 
-------------------------------
 (2048,2025,0)
(1 row)

postgres=# EXPLAIN (ANALYZE, COSTS OFF, TIMING OFF, SUMMARY OFF) SELECT count(*) FROM big;
                                 QUERY PLAN                                 
----------------------------------------------------------------------------
 Finalize Aggregate (actual rows=1.00 loops=1)
...
postgres=# SELECT count(*) AS buffers_of_big
postgres-# FROM pg_buffercache
postgres-# WHERE relfilenode = pg_relation_filenode('big');
 buffers_of_big 
----------------
            282
(1 row)
...
```psql
postgres=# SELECT current_setting('max_connections')::int
postgres-#      + current_setting('autovacuum_worker_slots')::int
postgres-#      + current_setting('max_worker_processes')::int
postgres-#      + current_setting('max_wal_senders')::int + 2 AS max_backends,
postgres-#        (SELECT setting::int FROM pg_settings WHERE name = 'shared_buffers')
postgres-#        / (current_setting('max_connections')::int
postgres-#           + current_setting('autovacuum_worker_slots')::int
postgres-#           + current_setting('max_worker_processes')::int
postgres-#           + current_setting('max_wal_senders')::int + 2 + 38) AS pin_limit_buffers;
 max_backends | pin_limit_buffers 
--------------+-------------------
          136 |                94
(1 row)

postgres=# SHOW io_combine_limit;
 io_combine_limit 
------------------
 128kB
(1 row)
...
`Buffers: shared read=10368 dirtied=10345 written=10086`도 눈여겨볼 만합니다. [앞에서 본 것](#테이블을-읽으면-shared-buffers에-페이지가-올라온다)과 같은 이유(처음 읽을 때 hint bit를 적음)로 읽은 페이지가 dirty가 되었고, ring의 칸을 다시 쓰려면 dirty 페이지를 먼저 디스크에 써야 해서 **읽기 쿼리가 약 10000페이지를 썼습니다.** 대량 적재 직후의 첫 SELECT가 유난히 느린 이유 중 하나입니다.

#### 운영에서는: 대량 적재 직후 첫 조회가 느리다

[shared buffers에 페이지가 올라오는 실습](#테이블을-읽으면-shared-buffers에-페이지가-올라온다)과 위 실습에서 본 것처럼, 새로 쓴 행을 처음 읽는 쿼리는 hint bit를 적느라 페이지를 dirty로 만들고, 큰 테이블이면 ring buffer 때문에 그 페이지를 곧바로 디스크에 씁니다. 대량 적재 뒤에 `VACUUM`(또는 `VACUUM (FREEZE)`)을 한 번 돌려 두면 이 작업이 미리 끝나서, 사용자 쿼리가 그 비용을 떠안지 않습니다. VACUUM은 [5편](/posts/postgresql/05-vacuum/)에서 다룹니다.
...
```psql
postgres=# SELECT name, level, pg_size_pretty(total_bytes) AS total, pg_size_pretty(used_bytes) AS used
postgres-# FROM pg_backend_memory_contexts
postgres-# ORDER BY total_bytes DESC
postgres-# LIMIT 8;
...
postgres=# SELECT count(*) AS contexts, pg_size_pretty(sum(total_bytes)) AS total
postgres-# FROM pg_backend_memory_contexts;
 contexts |  total  
----------+---------
      124 | 1495 kB
...
backend 개인 메모리에서 가장 큰 영향을 주는 설정이 `work_mem`입니다.
...
SET
postgres=# EXPLAIN (ANALYZE, COSTS OFF, TIMING OFF, SUMMARY OFF) SELECT * FROM big ORDER BY pad, id DESC;
                             QUERY PLAN                              
---------------------------------------------------------------------
...
#### 해시 테이블은 work_mem × hash_mem_multiplier까지 쓴다
...
postgres-# SELECT count(*) FROM big b1 JOIN big b2 USING (id);
                                QUERY PLAN                                
--------------------------------------------------------------------------
 Aggregate (actual rows=1.00 loops=1)
...
### work_mem은 노드마다 따로 잡힌다

주의할 점은 `work_mem`이 **쿼리 하나의 한도가 아니라 정렬이나 해시 노드 하나의 한도**라는 것입니다. 정렬 두 개와 해시 하나가 있는 쿼리는 `work_mem`의 몇 배를 쓸 수 있고, 그런 쿼리가 동시에 여러 개 돌면 곱절로 늘어납니다.

#### merge join의 정렬 두 개가 각각 work_mem을 쓴다
...
#### 운영에서는: work_mem이 메모리 폭증을 부르는 경우

위 실습에서 본 것처럼 `work_mem`은 정렬이나 해시 노드 하나의 한도입니다. 대략적인 최악의 사용량은 다음과 같습니다.
...
#### 임시 테이블은 temp_buffers를 쓴다

```psql
postgres=# CREATE TEMP TABLE tmp AS SELECT g AS id FROM generate_series(1, 100000) g;
SELECT 100000
postgres=# EXPLAIN (ANALYZE, COSTS OFF, TIMING OFF, SUMMARY OFF) SELECT count(*) FROM tmp;
                      QUERY PLAN                       
-------------------------------------------------------
 Aggregate (actual rows=1.00 loops=1)
...
postgres=# SELECT count(*) AS shared_buffers_used_by_tmp
postgres-# FROM pg_buffercache WHERE relfilenode = pg_relation_filenode('tmp');
...
postgres=# INSERT INTO big SELECT g, repeat('z', 100) FROM generate_series(1, 300000) g;
postgres=# SELECT wal_records, pg_size_pretty(wal_bytes) AS wal_bytes, wal_buffers_full FROM pg_stat_wal;
 wal_records | wal_bytes | wal_buffers_full 
-------------+-----------+------------------
      300003 | 46 MB     |             5368
(1 row)

postgres=# ALTER SYSTEM SET wal_buffers = '64kB';
```
